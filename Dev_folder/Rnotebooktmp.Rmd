...
- Research question to be answered here
- Attempt to answer prediction performance
- Attempt to understand covariance matrix fit
...
ff <- function(niets, p, N, noise_alpha) {
parms <- generate_params(p, 20, 1, 1, 0, alpha = noise_alpha, type = 'r')
Dat <- generate_data(N, parms)
X <- scale(Dat[,1:p], scale=F)
Y <- scale(Dat[,-(1:p)], scale=F)
fit1 <- o2m(X, Y, 1, 1, 0)
fitp1 <- PO2PLS(X, Y, 1, 1, 0, 1000, init_param = 'o')
fit2 <- o2m(X, Y, 1, 0, 0)
fitp2 <- PPLS_simult(X, Y, 1, 1000)

tops <- round(p/4)
outp <- rbind(
  O2PLS=sapply(1:1, function(i) order(-parms$W[,i]^2)[1:tops] %in% order(-fit1$W.[,i]^2)[1:tops] %>% mean),
  PO2PLS=sapply(1:1, function(i) order(-parms$W[,i]^2)[1:tops] %in% order(-fitp1$p$W[,i]^2)[1:tops] %>% mean),
  PLS=sapply(1:1, function(i) order(-parms$W[,i]^2)[1:tops] %in% order(-fit2$W.[,i]^2)[1:tops] %>% mean),
  PPLS=sapply(1:1, function(i) order(-parms$W[,i]^2)[1:tops] %in% order(-fitp2$e$W[,i]^2)[1:tops] %>% mean)
)
...
```{r plot tops, include=F}
# outp3 <- parallelsugar::mclapply(mc.cores=4, X = 1:48, FUN = ff, p=1e3, N = 50, noise_alpha=0.9)
# gc()
# outp4 <- parallelsugar::mclapply(mc.cores=4, X = 1:48, FUN = ff, p=1e4, N = 50, noise_alpha=0.9)
# gc()
load('rmd_outp34.RData')
```


- We additionally have $N=50$ and 90% noise level
- With PO2PLS we do better than the rest

```{r output tops}
outp3 %>% simplify2array %>% 
  apply(1,quantile, c(0.025, 0.5, 0.975))

outp4 %>% simplify2array %>% 
  apply(1,quantile, c(0.025, 0.5, 0.975))
...
covoutp0 <- covoutp %>% lapply(function(e) lapply(e, function(ee) (ee%*%diag(1/ee[5,],3))[1:4,])) %>%
  unlist %>% unname %>% array(dim = c(4,3,2,length(names(covoutp))), dimnames=c(dimnames(covoutp$`2500_20_0.5`$train[-5,]),list(names(covoutp[[1]])),list(names(covoutp))))
# covoutp0 <- covoutp %>% lapply(function(e) lapply(e, function(ee) (ee)[1:5,])) %>%
#   unlist %>% unname %>% array(dim = c(5,3,2,length(names(covoutp))), 
#     dimnames=c(dimnames(covoutp$`2500_20_0.5`$train),list(names(covoutp[[1]])),list(names(covoutp))))
